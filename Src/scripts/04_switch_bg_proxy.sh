...
OUTPUT3=$(curl "$IP:8080/v1/docker-flow-proxy/reconfigure?serviceName=demo_proxy-$2&servicePath=/hello&port=80")
echo $OUTPUT3
OUTPUT4=$(curl "$IP:8080/v1/docker-flow-proxy/remove?serviceName=demo_proxy-$1")
echo $OUTPUT4
echo 'DONE'
